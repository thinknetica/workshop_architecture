namespace :mongo_inpx do
  ...
  task :rebuild, [ :path ] => [ :environment ] do |task, args|
    ...
    Mongo::Book.destroy_all
    Seeds::Mongo::GenreLoad.call(filename: 'db/seeds/genres.yml')
    puts "Жанры #{Genre.count}"
    Seeds::Mongo::LanguageLoad.call(filename: 'db/seeds/languages.yml')
    ...
    Mongo::Folder.collection.insert_many(folders.flatten.uniq.map { |f| { name: f } })
    ...
    Parallel.map(Range.new(0, count - 1), in_processes: count) do |index|
      start = Time.zone.now
      Mongo::Books::Parse.call(books: chunks[index])
      finish = Time.zone.now
      puts format("Процесс %d: %0.2f сек", index, (finish - start))
    end
    puts "#{title} #{Mongo::Book.count}"
